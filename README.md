...
- 157 passing xUnit test cases in `tests/DragonCards.Tests` as of 2026-07-12, including all-mode launch/legality and mode-specific victory coverage, persistent Energy sources, SQLite profile migration/data controls, deterministic seeds, profile-sync contracts, quest, deck-code, matchmaking-contract, profile/deck persistence, collection discovery, LAN/direct-lobby, and aspect-fit coverage.
...
- Options screen with persisted display/audio/gameplay/accessibility settings in AppData, including reduced motion and an Interaction Pace selector (Cinematic, Natural, Quick, Fast); looping BGM and event/UI SFX are loaded and played by the desktop audio service.
...
- Core action results feed a testable `PresentationDirector` with exhaustive animation recipes, per-action grouping, parallel/coalesced secondary feedback, leftover-delta handling, whole-group skip, eased opacity/travel, activation-timed SFX, instance-safe live-card suppression, accurate moving-card endpoints, and user-selected playback pace. AI actions resolve one decision at a time after their presentation feedback completes.
